...
class AmsStreamingEndpointsTests(ScenarioTest):
    ...
    def test_ams_streaming_endpoint_show(self, storage_account_for_show):
        ...
        nonexistent_streaming_endpoint_name = self.create_random_name(prefix='se', length=20)
        self.kwargs.update({
            'nonexistent_streaming_endpoint_name': nonexistent_streaming_endpoint_name
        })
        with self.assertRaisesRegex(SystemExit, '3'):
            self.cmd('az ams streaming-endpoint show -g {rg} -a {amsname} -n {nonexistent_streaming_endpoint_name}')
