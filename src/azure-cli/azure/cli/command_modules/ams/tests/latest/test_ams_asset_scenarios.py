...
class AmsAssetTests(ScenarioTest):
    ...
    def test_ams_asset(self, storage_account_for_create, storage_account_for_asset):
        ...
        nonexistent_asset_name = self.create_random_name(prefix='asset', length=20)

        self.kwargs.update({
            'nonexistent_asset_name': nonexistent_asset_name
        })

        with self.assertRaisesRegex(SystemExit, '3'):
            self.cmd('az ams asset show -a {amsname} -g {rg} -n {nonexistent_asset_name}')
