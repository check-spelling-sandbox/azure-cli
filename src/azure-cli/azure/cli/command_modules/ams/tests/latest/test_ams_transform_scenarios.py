...
class AmsTransformTests(ScenarioTest):
    ...
    def test_ams_transform(self, storage_account_for_create):
        ...
        nonexistent_transform_name = self.create_random_name(prefix='transform', length=20)
        self.kwargs.update({
            'nonexistent_transform_name': nonexistent_transform_name
        })
        with self.assertRaisesRegex(SystemExit, '3'):
            self.cmd('az ams transform show -a {amsname} -n {nonexistent_transform_name} -g {rg}')
