...
def load_arguments(self, _):  # pylint: disable=too-many-locals, too-many-statements
    ...
    with self.argument_context('ams asset-filter') as c:
        ...
        c.argument('start_timestamp', arg_group='Presentation Time Range',
                   help='Applies to Video on Demand (VoD) or Live Streaming. This is a long value that represents an absolute start point of the stream. The value gets rounded to the closest next GOP start. The unit is the timescale, so a startTimestamp of 150000000 would be for 15 seconds. Use startTimestamp and endTimestamp to trim the fragments that will be in the playlist (manifest). For example, startTimestamp=40000000 and endTimestamp=100000000 using the default timescale will generate a playlist that contains fragments from between 4 seconds and 10 seconds of the VoD presentation. If a fragment straddles the boundary, the entire fragment will be included in the manifest.')
    ...
    with self.argument_context('ams streaming-policy') as c:
        ...
        c.argument('envelope_template', arg_group='Envelope Encryption', help='The KeyAcquisitionUrlTemplate is used to point to user specified service to delivery content keys.')
    ...
    with self.argument_context('ams live-event') as c:
        ...
        c.argument('key_frame_interval_duration', key_frame_interval_duration_arg_type, arg_group='Input', validator=validate_key_frame_interval_duration,
                   help='ISO 8601 timespan duration of the key frame interval duration in seconds. The value should be an integer in the range of 1 (PT1S or 00:00:01) to 30 (PT30S or 00:00:30) seconds.')
