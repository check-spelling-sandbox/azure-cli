...
class VMMonitorTestUpdateLinux(ScenarioTest):
    ...
    def test_vm_update_with_workspace_linux(self, resource_group):
        ...
        self.cmd('network nsg create -g {rg} -n {nsg}')
        # use new migrated log-analytics workspace create instead of monitor sdk
...
class VMSSCreateOptions(ScenarioTest):
    ...
    def test_vmss_with_resiliency_policy(self, resource_group):
        ...
        self.cmd('vmss create -g {rg} -n {vmss} --image {image} --vm-sku {sku} --admin-username vmtest --admin-password Test123456789# --enable-resilient-creation --disable-overprovision --upgrade-policy-mode Manual --orchestration-mode Uniform')
        self.cmd('vmss show -g {rg} -n {vmss}', checks=[
...
class VMSSCreateExistingOptions(ScenarioTest):
    ...
    def test_vmss_create_with_delete_option(self, resource_group):
        ...
        with self.assertRaisesRegex(InvalidArgumentValueError, message):
            self.cmd('vmss create -g {rg} -n {vmss} --image ubuntults --os-disk-delete-option delete --admin-username vmtest --orchestration-mode Uniform')
        with self.assertRaisesRegex(InvalidArgumentValueError, message):
            self.cmd('vmss create -g {rg} -n {vmss} --image ubuntults --data-disk-delete-option delete --admin-username vmtest --orchestration-mode Uniform')
...
class DiskBurstingTest(ScenarioTest):
    ...
    @ResourceGroupPreparer(name_prefix='cli_test_disk_bursting_', location='eastus')
