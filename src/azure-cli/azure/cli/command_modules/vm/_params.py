...
def load_arguments(self, _):
    ...
    with self.argument_context('vm create') as c:
        ...
        c.argument('boot_diagnostics_storage',
                   help='preexisting storage account name or its blob uri to capture boot diagnostics. Its sku should be one of Standard_GRS, Standard_LRS and Standard_RAGRS')
    ...
    for scope in ['vm create', 'vmss create']:
        with self.argument_context(scope) as c:
            ...
            c.argument('enable_integrity_monitoring', action='store_true', min_api='2020-12-01', help='Enable installing Microsoft proprietary and not security supported guest attestation extension and enabling System Assigned Identity for Trusted Launch enabled VMs and VMSS.')
        ...
        with self.argument_context(scope, arg_group='Storage') as c:
            ...
            c.argument('data_caching', options_list=['--data-disk-caching'], nargs='+',
                       help="storage caching type for data disk(s), including 'None', 'ReadOnly', 'ReadWrite', etc. Use a singular value to apply on all disks, or use `<lun>=<value1> <lun>=<value2>` to configure individual disk")
            c.argument('ultra_ssd_enabled', ultra_ssd_enabled_type)
